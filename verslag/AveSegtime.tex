...
Prior to starting any test or properly formalising our research questions, we note that segratation at \(n\%\) does not necessarily have to happen: 
If we consider \(n=100\) on the standard board with happiness \(1/3\). We will nearly never have complete segregation before the board reaches an equilibrium.
...
This guarantees that for any \(n\leq 100\), segregation at \(n\%\) takes place prior to the board reaching an equilibrium. Which leads to defining the research questions:
...
\end{enumerate}
To establish results regarding the first question, we consider different setups in testing.(NOG TOEVOEGEN).
